...
describe("calculateWashingtonTax", () => {
  describe("LTCG-only taxation", () => {
    it("does not tax wages (WA has no income tax)", () => {
      ...
      const result = calculateWashingtonTax(
        inputs,
        washingtonBrackets,
        washingtonLimits,
      );
    ...
    it("applies 9.9% starting at $1,000,001 - boundary test", () => {
      ...
      expect(result.ltcgTax).toBeCloseTo(50540.1, 2);
    });
  });
  ...
  describe("edge cases", () => {
    ...
    it("bracket breakdown shows correct structure", () => {
      ...
      expect(result.ltcgBracketBreakdown[1].taxForBracket).toBeCloseTo(
        50540,
        2,
      );
      ...
      expect(result.ltcgBracketBreakdown[2].taxForBracket).toBeCloseTo(
        19800,
        2,
      );
    });
...
describe.each(SUPPORTED_YEARS)(
  "calculateWashingtonTax - tax year %s",
  (year: TaxYear) => {
    ...
    it("uses correct LTCG exemption for year", () => {
      // Test that exemption threshold is applied
      const inputs = createDefaultInputs({
        federalIncome: 0,
        longTermCapitalGains: 278000,
        filingStatus: "single",
      ...
      const result = calculateWashingtonTax(
        inputs,
        data.washingtonBrackets,
        data.washingtonLimits,
      );

      // Both years use $278k exemption (2026 is placeholder)
      expect(result.totalTax).toBe(0);
      expect(result.taxableLTCG).toBe(0);
    });

    it("applies 7% on LTCG over exemption", () => {
      const inputs = createDefaultInputs({
        federalIncome: 0,
        longTermCapitalGains: 300000,
        filingStatus: "single",
      ...
      // Both years: $300k - $278k = $22k taxable @ 7% = $1,540
      const expectedTax = {
        "2025": 1540,
        "2026": 1540,
      };

      expect(result.ltcgTax).toBeCloseTo(expectedTax[year], 2);
    });

    it("applies 9.9% surtax on LTCG over $1M", () => {
      const inputs = createDefaultInputs({
        federalIncome: 0,
        longTermCapitalGains: 1200000,
        filingStatus: "single",
      ...
      // Tax structure unchanged between years
      // Tier 2: $722k @ 7% = $50,540
      // Tier 3: $200k @ 9.9% = $19,800
      // Total: $70,340
      const expectedTax = {
        "2025": 70340,
        "2026": 70340,
      ...
      expect(result.ltcgTax).toBe(expectedTax[year]);
    });
  },
);
