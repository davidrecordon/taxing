...
function calculateFicaTaxes(
  ...
  const socialSecurityWages = Math.min(
    wageIncome,
    ficaData.socialSecurity.wageBaseCap,
  );
  ...
  const medicareThreshold =
    ficaData.medicareAdditional.thresholds[filingStatus];
  const additionalMedicareWages = Math.max(0, wageIncome - medicareThreshold);
  const additionalMedicareTax =
    additionalMedicareWages * ficaData.medicareAdditional.rate;
...
export function calculateFederalTax(
  ...
  const grossOrdinaryIncome =
    inputs.federalIncome + effectiveSTCG + selfEmploymentIncome;
  ...
  const selfEmploymentTaxBreakdown =
    selfEmploymentIncome > 0 && ficaData
  ...
  const ordinaryIncomeOffset =
    remainingCarryover > 0 && inputs.federalIncome > 0
  ...
  const agiOrdinary = Math.max(
    0,
    grossOrdinaryIncome - shortTermLossCarryoverOffset - preTaxDeductions,
  );

  // Pre-deduction AGI for SALT cap threshold
  // Use conservative estimate (before LT carryover optimization)
  const preDeductionAgi =
    grossIncome - shortTermLossCarryoverOffset - preTaxDeductions;
  ...
  const qbiDeduction =
    selfEmploymentIncome > 0 && taxableOrdinaryIncomeBeforeQbi > 0
  ...
  const taxableOrdinaryIncome = Math.max(
    0,
    taxableOrdinaryIncomeBeforeQbi - qbiAmount,
  );
  ...
  const roomInZeroBracket = Math.max(
    0,
    zeroPercentThreshold - taxableOrdinaryIncome,
  );
  ...
  const longTermLossCarryoverOffset = Math.min(
    ltCarryover,
    ltcgInTaxedBrackets,
  );
  ...
  const niitTax =
    niitTaxableAmount > 0 ? niitTaxableAmount * federalLimits.niit.rate : 0;
  const niitBreakdown: NIITBreakdown | undefined =
    niitTax > 0
      ? {
          netInvestmentIncome,
          magiOverThreshold,
